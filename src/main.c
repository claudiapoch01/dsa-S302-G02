...
int main() {
    ...
    // Declaración de la estructura del Grafo
    Grafo mi_grafo;
    mi_grafo.nodos = NULL;
    mi_grafo.total_nodos = 0;

    // Variables de tipo double para guardar las coordenadas finales (latitud y longitud) del Origen y del Destino
    ...
    while (1) { // va a estar en el bucle hasta que se haga un break al cargar un mapa correctamente
        ...
        // Generamos el grafo de forma automática utilizando la lista de calles cargada
        mi_grafo = construir_grafo(lista_streets);

        break; // sale del bucle si el mapa se ha cargado bien
    }

    // muestra el número de casas, lugares cargados y elementos del grafo
